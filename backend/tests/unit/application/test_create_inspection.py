...
class TestCreateInspection:
    ...
    @pytest.fixture
    def valid_input(self, user_id):  # type: ignore[no-untyped-def]
    ...
    async def test_creates_inspection_with_draft_status(
        self, use_case: CreateInspection, valid_input: CreateInspectionInput
    ) -> None:
    ...
    async def test_saves_to_repository(
        self, use_case: CreateInspection, valid_input: CreateInspectionInput, fake_uow: FakeUnitOfWork
    ) -> None:
        await use_case.execute(valid_input)
